...
namespace Eds.Core.Utils
{
    ...
    public static class EmitUtil
    {
        ...
        public static FastCreateInstanceHandler GetInstanceCreator(Type type)
        {
            ...
            DynamicMethod dynamicMethod = new DynamicMethod(string.Empty, type, new Type[0], typeof(EmitUtil).Module);
        ...
        public static FastPropertySetHandler GetPropertySetter(PropertyInfo propInfo)
        {
            ...
            ilGenerator.Emit(OpCodes.Callvirt, propInfo.GetSetMethod());
            // terminates the call
            ilGenerator.Emit(OpCodes.Ret);
        ...
        private static void EmitCastToReference(ILGenerator ilGenerator, System.Type type)
        {
            if (type.IsValueType)
            {
                ...
                else
                {
                    ...
                    if (underType != null && underType.IsEnum)
                    {
                        //因可空枚举属性在赋值时出现过错误：类型转换异常，所以对可空枚举进行特殊处理
                        //Add by Arden 2012.11.26
                        ilGenerator.Emit(OpCodes.Unbox_Any, typeof(int?));
                    }
                    else
                    {
        ...
        public static object GetPropertyValue<T>(T Source, string Pname)
        {
            PropertyInfo info = Source.GetType().GetProperty(Pname);
